...
def test_teacher_integrates_held_high_level_velocity_continuously_between_updates():
    motion = _RecordingMotionDistributor()
    teacher = _teacher(motion, _RecordingWbcSolver())
    teacher.reset(_teacher_state(), seed=3)

    commands = [teacher.step(_teacher_state(step)) for step in range(4)]
    arm_targets = torch.stack([command.q_des[-7:] for command in commands])
    increments = arm_targets[1:] - arm_targets[:-1]

    assert torch.allclose(increments, torch.full_like(increments, 0.0005))
...
def test_failed_wbc_never_reuses_unverified_effort_and_advances_safety():
    ...
    assert torch.allclose(first.effort[-7:], torch.full((7,), 0.201, dtype=torch.float64))
