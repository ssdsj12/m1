...
Task 4 Fix Round 2 已完成；Task 5 仍未触碰。正常 `go2_pvcnn.tasks` 导入可完成并取得 `Isaac-M1-Panda-Smoke-v0` spec；旧环境仍在真正解析/创建其 cfg 时加载原模块，因此既有失败语义未被吞掉。当前资产验收为 Isaac-Sim-offline：15 个 bare `OmniPBR.mdl` 依赖由兼容 Isaac Sim 的内置 MDL resolver 提供，并不等价于脱离 Isaac Sim 材质库的严格项目自包含。另保留 PhysX 对 disabled Panda `root_joint` 的 disjointed-transform warning；一步内 mount 相对位移 `4.7527e-05 m`，低于 `1e-4 m` 门限，未观察到 snap。
