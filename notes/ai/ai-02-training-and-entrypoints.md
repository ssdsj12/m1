...
- After the approved task-local joint4 `-2.650 rad` amendment, GPU0 8×256 probing plus 8×1 and 64×10 smokes pass the physical gates. An isolated L0-C0 long launch is unlocked; see the [PD retune GPU log](../log/2026-08-25-m1-panda-folded-load-pd-retune-gpu.md).
- `Go2Pvcnn/scripts/m1_panda_folded_load_curriculum.py` is the only automatic promotion route. It launches train plus all three evals in strict stage order, revalidates the complete accepted SHA chain, and stops with a prior-checkpoint rollback pointer on the first failure. It never reduces difficulty and continues.
