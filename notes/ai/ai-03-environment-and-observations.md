...
- Wrapper exact-zeros Panda actions and writes fold position/zero-velocity targets before every step. The folded-load task alone uses joint4 `-2.650 rad` and shoulder `120/8`, while the global asset remains `-2.810 rad`/`80/4`; GPU0 256-step probing and both smoke levels pass the unchanged physical gates. See the [PD retune GPU log](../log/2026-08-25-m1-panda-folded-load-pd-retune-gpu.md).
